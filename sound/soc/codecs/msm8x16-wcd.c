...
#define CUSTOM_HANDMIC_SECONDARY_MIC_MARK 0
...
#if CUSTOM_HANDMIC_SECONDARY_MIC_MARK
static int s_handset_secondary_mic = 0;
#endif
...
#if CUSTOM_HANDMIC_SECONDARY_MIC_MARK
	if(dec_mux == 2 && strnstr(w->name, "DEC1 MUX", strlen(w->name))){
    	    s_handset_secondary_mic = 1; //mark 145 codec_reg value to 0x0 if handset-secondary mic
    	}else{
    	    s_handset_secondary_mic = 0;
    	}
#endif
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		*/
					...
					strnstr(w->name, external_text,
						strlen(w->name))) {
	#if CUSTOM_HANDMIC_SECONDARY_MIC_MARK
	        dev_dbg(codec->dev, "%s: s_handset_secondary_mic=%d\n", __func__, s_handset_secondary_mic);
			if(!s_handset_secondary_mic)
			    snd_soc_update_bits(codec,
					MSM8X16_WCD_A_ANALOG_TX_1_2_ATEST_CTL_2,
					0x01, 0x01);
			else{
			    ...
			    snd_soc_update_bits(codec,
                					MSM8X16_WCD_A_ANALOG_TX_1_2_ATEST_CTL_2,
                					0x01, 0x00); //mark 145 codec_reg value to 0x0 if handset-secondary mic
			}
	#else
	    snd_soc_update_bits(codec,
        					MSM8X16_WCD_A_ANALOG_TX_1_2_ATEST_CTL_2,
