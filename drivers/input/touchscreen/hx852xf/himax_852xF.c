...
#endif
//qijin add for tp_info show
snprintf(info, sizeof(info), "[FW]%#x%x%x,[ic]%s",
		private_ts->vendor_fw_ver_H, private_ts->vendor_fw_ver_L, private_ts->vendor_config_ver,TP_IC_NAME);
if(0 == panel_id_ret)
{
	init_tp_fm_info(0, info, TP_VENDOR_NAME);
}else{
	init_tp_fm_info(0, info, TP_VENDOR_NAME_1);
}
//qijin add end
